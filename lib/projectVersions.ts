...
export async function loadProjectVersions(
  ...
  return {
    ...
    chord: {
      ...
      error: chordRes.ok
        ? ""
        : chordData.error || "Failed to load chord versions",
    },
  };
}
...
export function normaliseProjectVersionData(
  ...
  const activeSongVersionId = songData.latest?.id || null;

  const latestChordVersion = activeSongVersionId
    ? chordVersions.find(
        (version) => version.song_version_id === activeSongVersionId,
      ) || null
    : null;
  ...
  return {
    ...
    chordVersions,
    activeSongVersionId,
    activeChordVersionId: latestChordVersion?.id || null,
