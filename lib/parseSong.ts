...
export function parsePerformanceSections(sheet: string): PerformanceSection[] {
  ...
  const isSectionHeader = (value: string) => {
    ...
    return /^(intro|verse(?:\s*\d+)?(?:\s*outro)?|pre[-\s]?chorus(?:\s*\d+)?|chorus(?:\s*\d+)?|final chorus|bridge|breakdown|outro|refrain|hook)(:)?$/i.test(
      trimmed,
    );
  };
...
export function classifyOrderedSongSection(
  label: string,
): OrderedSongSectionType {
...
export function buildOrderedPreviewBarsFromSections(
  ...
  const attachSection = (
    bars: Array<{ label: string; chord: string }>,
    section: OrderedSongSection,
  ): PreviewBar[] =>
    bars.map((bar) => ({
      label: section.label,
      chord: bar.chord,
  ...
  return sections.flatMap((section) => {
    ...
    if (section.type === "outro") {
      if (chorusBars.length >= 4) {
        return attachSection(
          chorusBars.slice(Math.max(0, chorusBars.length - 4)),
          section,
        );
      }
