...
export default function StructuredChordJsonEditor({
  ...
  return (
    <div ref={structuredChordJsonRef} className="mt-4">
      <h3 className="text-lg font-semibold mb-2">Structured Chord JSON</h3>

      <div className="mb-3 rounded border border-blue-700 bg-blue-900/30 px-3 py-2 text-xs text-blue-100">
        Tip: save a named chord version before changing extracted chords or
        trying a new arrangement.
      </div>
      ...
      {chordsText.trim() && !chordJsonIsValidObject && (
        <p className="mt-2 text-xs text-yellow-300">
          Enter a valid JSON object before saving, for example:{" "}
          {'{ "verse": "G | D | Em | C" }'}
        </p>
      )}
      ...
      {chordExtractionMessage && (
        <p className="mt-2 text-sm text-gray-300">{chordExtractionMessage}</p>
      )}
    </div>
  );
}
