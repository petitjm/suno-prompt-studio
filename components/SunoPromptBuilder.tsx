...
function getSunoEmotionalArc(performanceSheet: string) {
  ...
  if (hasChorus) {
    arcParts.push(
      "lift the chorus with stronger emotional release and hook focus",
    );
  }
...
function getSunoPerformanceNotes(performanceSheet: string) {
  ...
  if (hasChorus) {
    notes.push(
      "let choruses lift with stronger projection and clearer hook emphasis",
    );
  }

  if (hasBridge) {
    notes.push(
      "use the bridge for a subtle emotional shift rather than a dramatic reset",
    );
  }
  ...
  if (hasDuet) {
    notes.push(
      "use female harmony as support, not as a replacement for the lead vocal",
    );
  }
...
export default function SunoPromptBuilder({
  ...
  function getChordGuidanceSummary(structuredChordJson: string) {
    ...
    try {
      ...
      const chordPalette = Array.from(
        new Set(chordMatches.map((chord) => chord.trim()).filter(Boolean)),
      ).slice(0, 10);
      ...
      const likelyKey =
        chordPalette.includes("Em") ||
        chordPalette.some((chord) => chord.startsWith("Em"))
          ? "E minor / G major"
          : "the song’s main key";
  ...
  const [useCreationNotesAsMainDriver, setUseCreationNotesAsMainDriver] =
    useState(false);
  ...
  const [sunoResultRating, setSunoResultRating] = useState(
  ...
  const [justCopiedStructureGuide, setJustCopiedStructureGuide] =
    useState(false);
  const [justCopiedSectionArrangement, setJustCopiedSectionArrangement] =
    useState(false);
  ...
  const [justCopiedPerformanceNotes, setJustCopiedPerformanceNotes] =
    useState(false);
  const [justCopiedCreativeGuidePack, setJustCopiedCreativeGuidePack] =
    useState(false);
  const [justCopiedQuickPack, setJustCopiedQuickPack] = useState(false);
  const [justCopiedQuickCommaPack, setJustCopiedQuickCommaPack] =
    useState(false);
  const [justCopiedRevisionInputPack, setJustCopiedRevisionInputPack] =
    useState(false);
  const [justCopiedChordGuidance, setJustCopiedChordGuidance] = useState(false);
  const [justCopiedLyricsAndStyle, setJustCopiedLyricsAndStyle] =
    useState(false);
  const [justCopiedNegativeCommaList, setJustCopiedNegativeCommaList] =
    useState(false);
  const [justCopiedStyleAndVoice, setJustCopiedStyleAndVoice] = useState(false);
  const [activeVoicePresetFeedback, setActiveVoicePresetFeedback] =
    useState("");
  const [justCopiedSunoSettings, setJustCopiedSunoSettings] = useState(false);
  const [justCopiedStyleCommaList, setJustCopiedStyleCommaList] =
    useState(false);
  ...
  const [justCopiedProductionNotes, setJustCopiedProductionNotes] =
    useState(false);
  const [justCopiedRevisedFullPack, setJustCopiedRevisedFullPack] =
    useState(false);
  const [justCopiedStatusSummary, setJustCopiedStatusSummary] = useState(false);
  const [justCopiedStatusSummaryPreview, setJustCopiedStatusSummaryPreview] =
    useState(false);
  ...
  const [hasChangedSinceGeneration, setHasChangedSinceGeneration] =
    useState(false);
  ...
  const [justUsedGeneratedArrangement, setJustUsedGeneratedArrangement] =
    useState(false);
  ...
  const generateSunoPrompt = async () => {
    ...
    if (!performanceSheet.trim()) {
      setPromptMessage(
        "Add lyrics to the Song Sheet before generating Suno prompts.",
      );
      return;
    }
    ...
    try {
      ...
      setSunoStyleField(
        nextSunoStyleField.charAt(0).toUpperCase() +
          nextSunoStyleField.slice(1),
      );
  ...
  const applyPreset = (
    preset: "mpj-acoustic" | "modern-country" | "indie-folk" | "piano-ballad",
  ) => {
  ...
  const sunoVoiceInput = [sunoVoice, vocalDirection].filter(Boolean).join(". ");
  ...
  const sunoStyleCopyInput = [compactSunoStyleInput, chordGuidanceForStyle]
    .filter(Boolean)
    .join("\n\n");
  ...
  const getNegativePromptCommaList = () => {
    return negativePrompt
      ...
      .replace(
        /\bMaintain a natural, organic, human, and emotionally sincere sound\.?/gi,
        "unnatural sound, artificial sound, emotionally insincere sound",
      )
      .replace(
        /\bMaintain natural, organic, human, and emotionally sincere sound\.?/gi,
        "unnatural sound, artificial sound, emotionally insincere sound",
      )
      .replace(
        /\bsynthetic or dance production\b/gi,
        "synthetic production, dance production",
      )
      .replace(
        /\brobotic or artificial vocal effects\b/gi,
        "robotic vocal effects, artificial vocal effects",
      )
      .replace(
        /\bnovelty or comedic vocal styles\b/gi,
        "novelty vocals, comedic vocal styles",
      )
  ...
  const currentPresetDirection = [
    stylePrompt.trim()
      ? `Style: ${stylePrompt.trim().split("\n")[0]}`
      : "Style: not set",
    sunoVoiceInput.trim()
      ? `Voice: ${sunoVoiceInput.trim().split("\n")[0]}`
      : "Voice: not set",
  ].join(" · ");
  ...
  const sunoHandoffPack = [
    ...
    sunoQuickPack,
    "",
    "ADDITIONAL CORE REFERENCE FIELDS:",
    "Use these as supporting guidance. The main Suno paste targets are still LYRICS and STYLE above.",
    "",
    "SUNO STYLE PROMPT:",
    ...
    <section className="mt-6 p-4 rounded bg-gray-900 border border-gray-700 max-w-5xl">
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-gray-400">
          Turn the current song into a copy-ready Suno production handoff.
        </p>
      </div>

      <details className="mb-4 rounded border border-gray-700 bg-gray-800 p-3">
        ...
        <p className="mt-2 text-xs text-gray-500">
          These are the current source lyrics used to build the Suno handoff.
        </p>

        {performanceSheet.trim() ? (
          <pre className="mt-3 max-h-80 overflow-y-auto whitespace-pre-wrap rounded bg-gray-950 p-3 text-sm leading-relaxed text-gray-300">
            {performanceSheet}
          </pre>
        ) : (
          <div className="mt-3 rounded border border-gray-700 bg-gray-950 px-3 py-4 text-sm text-gray-400">
            No source lyrics are currently loaded. Return to Song to add or load
            a song before generating Suno prompts.
          </div>
        )}
      </details>
      ...
      <div className="mb-5 rounded border border-gray-700 bg-gray-900/40 p-4">
        ...
        <p className="mb-3 text-xs text-gray-400">
          Choose a starting production style before generating or refining Suno
          prompts.
        </p>
        ...
        {activePresetFeedback && (
          <p className="mt-2 text-xs text-green-300">{activePresetFeedback}</p>
        )}
      </div>
      ...
      <div className="mb-5 rounded border border-gray-700 bg-gray-900/40 p-4">
        ...
        <p className="mb-3 text-xs text-gray-400">
          Choose the vocal/persona direction and gender setting for the Suno
          handoff. Use Reset MPJ preset to return to the default acoustic MPJ
          starting point.
        </p>
        ...
        <div className="mt-3 rounded border border-gray-700 bg-gray-950/50 p-3">
          <p className="text-xs text-gray-400">Current preset direction</p>
          <p className="mt-1 text-sm text-gray-200">{currentPresetDirection}</p>
        </div>
        ...
        <div className="mb-5 rounded border border-gray-700 bg-gray-900/60 p-3">
          ...
          <p className="mb-3 text-xs text-gray-400">{sunoStatusSummary}</p>
              ...
              <div>
                ...
                <div className="grid gap-2 text-xs text-gray-300 sm:grid-cols-2 xl:grid-cols-3">
                  ...
                  <div>
                    ...
                    <span className="text-gray-100">{sunoResultRating}</span>
                  </div>

                  <div>
                    <span className="text-gray-500">Revision focus:</span>{" "}
                    <span className="text-gray-100">{revisionFocus}</span>
                  </div>
                  ...
                  <div>
                    <span className="text-gray-500">Handoff state:</span>{" "}
                  ...
                  <button
                    ...
                    title={sunoStatusSummaryPreviewCopyTitle}
                    className="mt-2 rounded border border-gray-700 px-3 py-1.5 text-xs text-gray-200 hover:bg-gray-800"
                  >
        ...
        <div className="mb-3">
          ...
          <p className="text-sm text-gray-400">
            Main editable prompt fields used to generate and refine your Suno
            handoff.
          </p>
          <p className="mt-2 text-xs text-gray-500">{coreFieldStatus}</p>
        </div>
        ...
        <div className="grid gap-4">
          <label className="block">
            ...
            <p className="mt-1 text-xs text-gray-400">
              Main musical identity: genre, mood, vocal character,
          ...
          <label className="block">
            ...
            <textarea
              ...
              onChange={(e) => {
                setVocalDirection(e.target.value);
                clearGenerationState();
              }}
              rows={3}
              className="w-full px-3 py-2 rounded bg-gray-800 text-white border border-gray-700"
            />
            <p className="mt-1 text-xs text-gray-400">
              Describes the lead vocal tone, delivery, accent, emotion, and
              chorus lift.
            </p>
          </label>
          ...
          <label className="block">
            ...
            <textarea
              ...
              onChange={(e) => {
                setArrangementNotes(e.target.value);
                clearGenerationState();
              }}
              rows={3}
              className="w-full px-3 py-2 rounded bg-gray-800 text-white border border-gray-700"
            />
            <p className="mt-1 text-xs text-gray-400">
              Controls the song build, instrumentation, dynamics, and production
              approach.
            </p>
          </label>
          ...
          <label className="block">
            ...
            <textarea
              ...
              onChange={(e) => {
                setIntroSoloOutro(e.target.value);
                clearGenerationState();
              }}
              rows={3}
              className="w-full px-3 py-2 rounded bg-gray-800 text-white border border-gray-700"
            />
            <p className="mt-1 text-xs text-gray-400">
              Optional guidance for openings, instrumental breaks, solos,
          ...
          <label className="block">
            ...
            />
            <p className="mt-1 text-xs text-gray-400">
              Describes unwanted sounds, styles, vocal effects, instruments, or
              production choices.
            </p>
          </label>
        </div>
        ...
        <div className="space-y-4">
          <div className="rounded border border-gray-700 bg-gray-900/50 p-4">
            ...
            <p className="mb-3 text-xs text-gray-400">
              Generate creates new Suno prompts. Copy Suno handoff copies the
              main paste-ready pack, including Suno 5.5 Advanced paste-order
              guidance, lyrics, style, voice, settings, negative prompt, and
              additional core reference fields for arrangement, vocal, and intro
              / solo / outro guidance. Reset restores the Suno prompt fields to
              their defaults without changing the song sheet or saved versions.
              Clear session removes revision notes, revision history, generation
              messages, and handoff session state without changing the song
              sheet or resetting the core prompt defaults.
            </p>
            <div className="mb-3 space-y-1 text-xs">
              ...
              <p className="text-gray-500">{sunoHandoffGeneratedAtText}</p>
            </div>
            <details className="mb-3 rounded border border-yellow-800 bg-yellow-950/20 p-3">
              ...
              <p className="mt-2 text-xs text-yellow-100">
                Copy Suno handoff only copies the current paste-ready pack to
                your clipboard, including Suno 5.5 paste-order guidance and the
                current visible additional core reference fields. It does not
                send anything to Suno, save a version, or change the song sheet.
                Reset affects the visible Suno prompt fields only. It does not
                delete projects, saved song versions, saved chord versions, or
                the song sheet. Clear session removes revision notes, revision
                history, and generation messages without changing the song
                sheet.
              </p>
            </details>
            <div className="flex flex-wrap gap-3">
              ...
              >
                {justResetDefaults
                  ? sunoResetDefaultsCopiedLabel
                  : sunoResetDefaultsButtonLabel}
              </button>
              <button
                type="button"
          ...
          <div className="mt-3 rounded border border-blue-800 bg-blue-950/30 p-3">
            ...
            <p className="text-sm text-blue-100">{sunoWorkflowHint}</p>
          </div>
          ...
          {hasChangedSinceGeneration && (
            <div className="mt-3 rounded border border-yellow-800 bg-yellow-950/30 p-3">
              ...
              <p className="text-xs text-yellow-100">
                The current Suno handoff has been edited since the last
                generated prompt. Generate again if you want AI-refreshed
                guidance, or copy the edited handoff if you intentionally
                changed the fields yourself.
              </p>
          ...
          {generatedFromSummary && (
            <div className="mt-3 rounded border border-gray-700 bg-gray-900/50 p-3">
              ...
              <ul className="list-disc space-y-1 pl-5 text-xs text-gray-400">
                <li>
                  Click {sunoHandoffButtonLabel}, then paste the Lyrics and
                  Style sections into Suno 5.5 Advanced mode.
                </li>
                <li>
                  Use Quick copy only if you want to paste individual fields
                  separately.
                </li>
                <li>
                  After listening in Suno, open Revision Controls and note what
                  worked, what failed, and what should change next.
                </li>
              </ul>
            </div>
          )}
          ...
          {generatedFromSummary && !creationNotes.trim() && (
            <div className="mt-3 rounded border border-yellow-800 bg-yellow-950/30 p-3">
              ...
              <p className="text-xs text-yellow-100">
                After listening to the Suno result, open Revision Controls and
                add short notes such as: vocal too soft, guitar needs more
                presence, intro too long, chorus worked well.
              </p>
            </div>
          )}
          ...
          {generatedFromSummary && creationNotes.trim() && (
            <div className="mt-3 rounded border border-green-800 bg-green-950/30 p-3">
              ...
              <p className="text-xs text-green-100">
                Your creation notes are ready to guide the next Suno prompt
                generation. Use Revision Controls to adjust focus, rating, keep,
          ...
          {generatedFromSummary && (
            <div className="mt-3 rounded border border-gray-700 bg-gray-950/50 p-3">
              ...
              <p className="mb-3 text-xs text-gray-400">
                Use the generated style, voice, or arrangement guidance as the
        ...
        <div className="mt-5 rounded border border-gray-700 bg-gray-900/50 p-4">
          ...
          <p className="mb-3 text-xs text-gray-400">
            Secondary copy options for pasting individual Suno fields
            separately. For the normal workflow, use the main Copy Suno handoff
            button above.
          </p>
          <div className="flex flex-wrap gap-3">
            <button
        ...
        <details className="rounded border border-gray-700 bg-gray-900/50 p-4">
          ...
          <p className="mt-3 text-xs text-gray-400">
            Secondary copy options for revision notes, full packs, creative
            guides, and detailed reference material.
          </p>
        ...
        <details className="mt-5 rounded border border-gray-700 bg-gray-900/60 p-4">
          ...
          <p className="mt-2 mb-4 text-sm text-gray-400">
            Copy-ready fields for pasting into Suno 5.5 Advanced mode, including
            lyrics, style, voice, settings, chord guidance, and negative prompt
            options.
          </p>
          ...
          <div className="space-y-4">
            ...
            <label className="block">
              ...
              />
              <p className="mt-1 text-xs text-gray-400">
                Paste this into Suno&apos;s Style field. Prompt length and chord
                guidance are applied here.
              </p>
            </label>
            ...
            <label className="block">
              ...
              <p className="mt-1 text-xs text-gray-400">
                Suno treats chord text as guidance, not a guaranteed chord
                chart.
              </p>
            </label>
            ...
            <label className="block">
              ...
              />
              <p className="mt-1 text-xs text-gray-400">
                Use these as reference notes; they are usually too detailed for
                Suno&apos;s Style field.
              </p>
            </label>
        ...
        <details className="mt-5 rounded border border-gray-700 bg-gray-900/60 p-4">
          ...
          <p className="mt-2 mb-4 text-sm text-gray-400">
            Structure, arrangement, hook, emotional arc, and performance
            guidance generated from the current song sheet.
          </p>
          ...
          <div className="space-y-4">
            <label className="block">
              ...
              />
              <p className="mt-1 text-xs text-gray-400">
                Use this as optional structure guidance for Suno or for prompt
                revision.
              </p>
            </label>
            <label className="block">
            ...
            <label className="block">
              ...
              />
              <p className="mt-1 text-xs text-gray-400">
                Combined structure, arrangement, hook, emotional arc, and
        ...
        <details className="mt-5 rounded border border-gray-700 bg-gray-900/60 p-4">
          ...
          <p className="mt-2 mb-4 text-sm text-gray-400"></p>
          <p className="mt-2 text-sm text-gray-400">
            Preview the ready-made copy blocks used for Suno handoff, quick
            testing, comma-style prompts, and revision workflows. These previews
            always reflect the current visible fields, including any manual
          ...
          <label className="block">
            <span
              className={`block text-sm font-medium mb-1 ${sunoHandoffStateClassName}`}
            >
              {sunoHandoffPackDisplayTitle}
            </span>
            <textarea
            ...
            <p className="mt-1 text-xs text-gray-400">
              Preview of the main pack copied by the Primary actions “Copy Suno
              handoff” button, including Suno 5.5 Advanced paste-order guidance
              and additional core reference fields.
            </p>
          </label>
          <div className="space-y-4">
            <label className="block">
              ...
              />
              <p className="mt-1 text-xs text-gray-400">
                Compact everyday pack with lyrics, style, voice, settings, and
            ...
            <label className="block">
              ...
              />
              <p className="mt-1 text-xs text-gray-400">
                Compact everyday pack using comma-list versions of the style and
            ...
            <label className="block">
              ...
              />
              <p className="mt-1 text-xs text-gray-400">
                Combined revision notes, current prompts, and creative guidance
                for refining a Suno result.
              </p>
            </label>
          </div>
        </details>
        ...
        <details className="mt-5 rounded border border-gray-700 bg-gray-900/60 p-4">
          ...
          <p className="mt-2 mb-4 text-sm text-gray-400">
            Full combined prompt assembled from the current style, vocal,
            arrangement, intro / solo / outro, and negative prompt fields.
          </p>
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        ...
        {promptMessage && (
          <p className="mt-3 text-sm text-gray-400">{promptMessage}</p>
        )}
      </div>
    </section>
  );
