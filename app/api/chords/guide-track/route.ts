...
const GUIDE_TRACK_TIMEOUT_MS = 300_000;
...
`.trim();
    ...
    logOpenAIUsage(
      `chords/guide-track model=${GUIDE_TRACK_MODEL}`,
      startedAt,
      completion,
    );
    ...
    if (isTimeoutError(error)) {
      return NextResponse.json(
        {
          error:
            "Guide track plan generation timed out after 5 minutes. Please try again, or shorten the lyrics.",
        },
        { status: 504 },
      );
    }
