...
function normalizeChordPlacement(
  item: unknown,
): AudioPreviewChordPlacement | null {
  if (!item || typeof item !== "object" || Array.isArray(item)) {
    return null;
  }
...
function normalisePreviewSectionLabel(value: string) {
  return value
    .trim()
    .toLowerCase()
    .replace(/([a-z])(\d)/g, "$1 $2")
...
function isDuplicateSectionLabelLyric(line: AudioPreviewSongSheetLine) {
  const sectionLabel = normalisePreviewSectionLabel(line.section || "");
  const lyricLabel = normalisePreviewSectionLabel(line.lyric || "");

  if (!sectionLabel || !lyricLabel) {
    return false;
  }

  return (
    lyricLabel === sectionLabel ||
    (sectionLabel.startsWith("verse ") && lyricLabel === "verse")
  );
}
...
function buildPreviewSongSheetText(lines: AudioPreviewSongSheetLine[]) {
  ...
  lines.forEach((line) => {
    ...
    if (!isDuplicateSectionLabelLyric(line)) {
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (
      !Array.isArray(body.songsheetLines) ||
      body.songsheetLines.length === 0
    ) {
      return NextResponse.json(
    ...
    const previewSongSheetText = buildPreviewSongSheetText(
      normalizedSongSheetLines,
    );

    const previewSectionGuideText = buildPreviewSectionGuideText(
