...
export async function POST(req: Request) {
  try {
    ...
    const songVersionId = body.songVersionId || body.song_version_id || null;

    if (songVersionId) {
      const { data: songVersion, error: songVersionError } = await supabase
        .from("song_versions")
      ...
      if (songVersionError || !songVersion) {
        return NextResponse.json(
          { error: "Song version not found for this project" },
          { status: 404 },
        );
      }
    }
