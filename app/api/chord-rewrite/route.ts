...
`
    ...
    console.error('Artist DNA lookup failed in chord rewrite route:', err)
    return ''
  }
}
...
- Use the artist DNA where helpful, especially for vocal range, style, harmonic colour, and live suitability.
...
`
    ...
    if (body.project_id) {
      ...
      if (user) {
        ...
        if (projectUpdateError) {
          console.error('projects updated_at bump failed after chord rewrite:', projectUpdateError)
        }
      }
    }
    ...
    console.error('Chord rewrite route failure:', err)
    return NextResponse.json(
