...
import { countLyricLines } from "@/lib/rewriteText";

import { requestRewrite } from "@/lib/rewriteApi";

import { buildRewriteInstruction, rewritePresets } from "@/lib/rewritePrompts";
...
export default function Page() {
  ...
  const [previewSection, setPreviewSection] =
    useState<PreviewSectionKey>("verse");
  const [previewPattern, setPreviewPattern] =
    useState<PreviewPattern>("ballad_strum");
  ...
  const [justExtractedAndRemovedChords, setJustExtractedAndRemovedChords] =
    useState(false);
  ...
  const [audioPreviewRenderMessage, setAudioPreviewRenderMessage] =
    useState("");
  const [audioPreviewRenderJob, setAudioPreviewRenderJob] = useState<Record<
    string,
    unknown
  > | null>(null);
  const [audioPreviewRenderResponse, setAudioPreviewRenderResponse] =
    useState("");
  const [audioPreviewDryRunRenderPlan, setAudioPreviewDryRunRenderPlan] =
    useState<Record<string, unknown> | null>(null);
  const [dryRunRenderManifestValidation, setDryRunRenderManifestValidation] =
    useState<Record<string, unknown> | null>(null);
  const [dryRunRenderManifest, setDryRunRenderManifest] = useState<Record<
    string,
    unknown
  > | null>(null);
  const [dryRunCueSheetValidation, setDryRunCueSheetValidation] =
    useState<Record<string, unknown> | null>(null);
  const [dryRunRenderPlanValidation, setDryRunRenderPlanValidation] =
    useState<Record<string, unknown> | null>(null);
  const [dryRunHandoffBundle, setDryRunHandoffBundle] = useState<Record<
    string,
    unknown
  > | null>(null);
  const [dryRunHandoffBundleValidation, setDryRunHandoffBundleValidation] =
    useState<Record<string, unknown> | null>(null);
  const [dryRunArtifactPackage, setDryRunArtifactPackage] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const [audioPreviewPlan, setAudioPreviewPlan] = useState<Record<
    string,
    unknown
  > | null>(null);
  const [audioPreviewRenderPrompt, setAudioPreviewRenderPrompt] = useState("");
  const [audioPreviewMeta, setAudioPreviewMeta] = useState<Record<
    string,
    unknown
  > | null>(null);
  const [audioPreviewRendererPayload, setAudioPreviewRendererPayload] =
    useState<Record<string, unknown> | null>(null);
  const [
    audioPreviewRendererPayloadValidation,
    setAudioPreviewRendererPayloadValidation,
  ] = useState<Record<string, unknown> | null>(null);
  const [audioPreviewSongSheetText, setAudioPreviewSongSheetText] =
    useState("");
  const [audioPreviewSectionGuideText, setAudioPreviewSectionGuideText] =
    useState("");
  ...
  const [justCopiedChordPracticePack, setJustCopiedChordPracticePack] =
    useState(false);
  const [justCopiedChordSheet, setJustCopiedChordSheet] = useState(false);
  const [justCopiedPlacedSongSheet, setJustCopiedPlacedSongSheet] =
    useState(false);
  const [justCopiedPerformanceIntent, setJustCopiedPerformanceIntent] =
    useState(false);
  const [
  ...
  const [
    ...
    setJustCopiedPerformanceDesignNotes,
  ] = useState(false);
  const [justCopiedGuideTrackPlan, setJustCopiedGuideTrackPlan] =
    useState(false);
  const [justCopiedFullPerformancePack, setJustCopiedFullPerformancePack] =
    useState(false);
  const [justCopiedSongsheetReview, setJustCopiedSongsheetReview] =
    useState(false);
  const [justCopiedAudioGuidePrompt, setJustCopiedAudioGuidePrompt] =
    useState(false);
  const [
    justCopiedAudioPreviewDryRunPlan,
    setJustCopiedAudioPreviewDryRunPlan,
  ] = useState(false);
  const [
    justCopiedAudioPreviewSectionGuide,
    setJustCopiedAudioPreviewSectionGuide,
  ] = useState(false);
  const [
    justCopiedAudioPreviewRendererPayload,
    setJustCopiedAudioPreviewRendererPayload,
  ] = useState(false);
  const [justCopiedAudioGuideSummary, setJustCopiedAudioGuideSummary] =
    useState(false);
  const [justCopiedAudioPreviewSpec, setJustCopiedAudioPreviewSpec] =
    useState(false);
  const [justCopiedAudioRenderPrompt, setJustCopiedAudioRenderPrompt] =
    useState(false);
  const [justCopiedAudioRenderSteps, setJustCopiedAudioRenderSteps] =
    useState(false);
  const [justCopiedAudioPreviewSongSheet, setJustCopiedAudioPreviewSongSheet] =
    useState(false);
  const [justCopiedAudioPreviewChecklist, setJustCopiedAudioPreviewChecklist] =
    useState(false);
  const [justCopiedAudioPreviewCueSheet, setJustCopiedAudioPreviewCueSheet] =
    useState(false);
  const [
    justCopiedAudioPreviewRenderManifest,
    setJustCopiedAudioPreviewRenderManifest,
  ] = useState(false);
  const [justCopiedGenerationUsage, setJustCopiedGenerationUsage] =
    useState(false);
  ...
  const [lastAppliedTransposeSnapshot, setLastAppliedTransposeSnapshot] =
    useState<{
      chordsText: string;
  ...
  const performanceSectionRefs = React.useRef<
  ...
  const syncPreviewScroll = (source: "left" | "right") => {
    const src =
      source === "left" ? previewLeftRef.current : previewRightRef.current;
    const tgt =
      source === "left" ? previewRightRef.current : previewLeftRef.current;
    if (!src || !tgt) return;
  ...
  const syncCompareScroll = (source: "left" | "right") => {
    ...
    const src =
      source === "left" ? compareLeftRef.current : compareRightRef.current;
    const tgt =
      source === "left" ? compareRightRef.current : compareLeftRef.current;
  ...
  const [rewriteTarget, setRewriteTarget] = useState<"left" | "right" | "main">(
    "right",
  );
  ...
  const scrollPerformanceToBarIndex = (
    ...
    const safeBarIndex = Math.max(
      0,
      Math.min(barIndex, previewBarMeta.length - 1),
    );
    ...
    const currentSectionStartBar =
      previewBarMeta.find((bar) => bar.sectionId === currentSection.id)
        ?.barIndex ?? safeBarIndex;

    const nextSectionStartBar = nextSection
      ? (previewBarMeta.find((bar) => bar.sectionId === nextSection.id)
          ?.barIndex ?? previewBarMeta.length)
      : previewBarMeta.length;

    const sectionBarSpan = Math.max(
      1,
      nextSectionStartBar - currentSectionStartBar,
    );
    ...
    const currentTop = Math.max(
      0,
      currentSectionEl.offsetTop - anchorOffset - 12,
    );
  ...
  const compareLeftSong =
    songVersions.find((v) => v.id === compareLeftSongId) || null;
  const compareRightSong =
    songVersions.find((v) => v.id === compareRightSongId) || null;
  ...
  const loadProjects = async (preferredProjectId?: string) => {
    try {
      ...
      const nextProjects: Project[] = Array.isArray(data.projects)
        ? data.projects
        : [];
  ...
  const loadProjectData = async (
    ...
    try {
      ...
      const normalisedProjectData = normaliseProjectVersionData(
        songData,
        chordData,
      );
      ...
      setCompareRightSongId(
        normalisedProjectData.initialCompareSongIds.rightId,
      );
  ...
  const buildChordGenerationUsageCopyText = () => {
    ...
    return [
      ...
      ...(usageWarning.hasWarning
        ? ["USAGE WARNING", "", usageWarning.label, usageWarning.detail, ""]
        : []),
      "STAGES",
      "",
      ...entries.flatMap((entry, index) => {
        const route =
          typeof entry.route === "string" ? entry.route : "Unknown route";
        const model =
          typeof entry.model === "string" ? entry.model : "Unknown model";
        const duration =
  ...
  const getChordGenerationHistorySummary = () => {
    ...
    const totalInputTokens = entries.reduce((total, entry) => {
      return (
        total + (typeof entry.inputTokens === "number" ? entry.inputTokens : 0)
      );
    }, 0);
    ...
    const totalTokens = entries.reduce((total, entry) => {
      return (
        total + (typeof entry.totalTokens === "number" ? entry.totalTokens : 0)
      );
    }, 0);
  ...
  const copyChordGenerationUsage = async () => {
    ...
    if (!copyText) {
      setChordExtractionMessage(
        "No staged generation usage available to copy.",
      );
      return;
    }
  ...
  const resetOrUndoChordTranspose = () => {
    ...
    setLastAppliedTransposeSnapshot(null);
    setChordExtractionMessage(
      "Applied transpose undone. Review and save when ready.",
    );
    setProjectMessage("");
  ...
  const applyTransposeToChordEditor = () => {
    ...
    if (originalKey) {
      nextRecord.key = transposeChordSymbol(
        originalKey,
    ...
    setChordTransposeSemitones(0);
    setChordExtractionMessage(
      "Transpose applied to chord editor. Review and save as a new chord version.",
    );
    setProjectMessage("");
  };
  ...
  const requestAudioPreview = async () => {
    ...
    try {
      ...
      setAudioPreviewSectionGuideText(
        typeof result.sectionGuideText === "string"
  ...
  const getRealRenderRouteReceivedConfigurationSummary = () => {
    ...
    clickTrackDownloadArtifactBytesIncluded:
      ...
      recommendedFirstRenderer:
        typeof receivedConfigurationSummary.recommendedFirstRenderer ===
        "string"
      ...
      recommendedFirstProvider:
        typeof receivedConfigurationSummary.recommendedFirstProvider ===
        "string"
  ...
  const getBlockedRealRenderRouteTestPassed = () => {
    ...
    const receivedContractSummary = getRealRenderRouteReceivedContractSummary();

    const receivedContractCheck = getRealRenderRouteReceivedContractCheck();
  ...
  const getRenderStepValue = (step: Record<string, unknown>, key: string) => {
  ...
  const buildAudioRenderStepsCopyText = () => {
    ...
    return [
      ...
      ...audioPreviewRenderSteps.flatMap((step, index) => {
        const stepNumber =
          getRenderStepValue(step, "step") || String(index + 1);
        const section =
          getRenderStepValue(step, "section") || `Section ${stepNumber}`;
  ...
  const copyAudioPreviewArtifactPackage = async () => {
    if (!dryRunArtifactPackage) {
      setAudioPreviewRenderMessage(
        "No audio preview dry-run artefact package available to copy.",
      );
      return;
    }
    ...
    const firstRealRenderPlanSummary = getDryRunFirstRealRenderPlanSummary();
    ...
    const copyText = [
      ...
      ...(guideTrackRenderRecipeSummary
        ? [
            ...
            guideTrackRenderRecipeSummary.rendererRequirement
              ...
              : "",
            "",
            ...(guideTrackRenderRecipeSummary.musicalBed.supportInstruments
              .length > 0
              ? [
                  "Support instruments:",
      ...
      ...(clickTrackRenderRecipeSummary
        ? [
            ...
            `Click sound: ${
              clickTrackRenderRecipeSummary.clickSound.subdivision ||
              "Not declared"
            }${
      ...
      ...(vocalGuideRenderRecipeSummary
        ? [
            ...
            ...(vocalGuideRenderRecipeSummary.activationRequirements.length > 0
              ...
              : []),
            ...(vocalGuideRenderRecipeSummary.melodySource.acceptedSources
              .length > 0
              ? [
      ...
      ...(firstRealRenderPlanSummary
        ? [
            ...
            `Strategy: ${
              firstRealRenderPlanSummary.rendererStrategy.strategyType ||
              "Unknown"
            }`,
            `Implementation status: ${
              firstRealRenderPlanSummary.rendererStrategy
            ...
            ...(firstRealRenderPlanSummary.notes.length > 0
              ? [
                  "Notes:",
                  ...firstRealRenderPlanSummary.notes.map(
                    (note) => `- ${note}`,
                  ),
                  "",
                ]
        ...
        ? [
            ...
            `Format candidate: ${
              realRenderConfigurationSummary.outputFormat
                .recommendedFirstFormat || "not declared"
            } (${
            ...
            realRenderConfigurationSummary.storage.reason
              ...
              : "",
            ...(realRenderConfigurationSummary.storage.mustRemainBlockedUntil
              .length > 0
              ? [
    ...
    try {
      ...
      setAudioPreviewRenderMessage(
        "Audio preview dry-run artefact package copied.",
      );
      ...
      setAudioPreviewRenderMessage(
        "Could not copy audio preview dry-run artefact package.",
      );
    }
  };
  ...
  const copyAudioPreviewHandoffBundle = async () => {
    if (!dryRunHandoffBundle) {
      setAudioPreviewRenderMessage(
        "No audio preview dry-run handoff bundle available to copy.",
      );
      return;
    }
    ...
    try {
      ...
      setAudioPreviewRenderMessage(
        "Audio preview dry-run handoff bundle copied.",
      );
      ...
      setAudioPreviewRenderMessage(
        "Could not copy audio preview dry-run handoff bundle.",
      );
    }
  };
  ...
  const copyAudioPreviewRenderManifest = async () => {
    if (!dryRunRenderManifest) {
      setAudioPreviewRenderMessage(
        "No audio preview dry-run render manifest available to copy.",
      );
      return;
    }
    ...
    try {
      ...
      setAudioPreviewRenderMessage(
        "Audio preview dry-run render manifest copied.",
      );
      ...
      setAudioPreviewRenderMessage(
        "Could not copy audio preview dry-run render manifest.",
      );
    }
  };
  ...
  const copyAudioPreviewCueSheet = async () => {
    if (!audioPreviewDryRunRenderPlan) {
      setAudioPreviewRenderMessage(
        "No audio preview dry-run cue sheet available to copy.",
      );
      return;
    }
    ...
    try {
      ...
      setAudioPreviewRenderMessage(
        "Could not copy audio preview dry-run cue sheet.",
      );
    }
  };
  ...
  const copyAudioPreviewDryRunPlan = async () => {
    if (!audioPreviewDryRunRenderPlan) {
      setAudioPreviewRenderMessage(
        "No audio preview dry-run render plan available to copy.",
      );
      return;
    }
    ...
    try {
      ...
      setAudioPreviewRenderMessage(
        "Could not copy audio preview dry-run render plan.",
      );
    }
  };
  ...
  const copyAudioPreviewRendererPayload = async () => {
    if (!audioPreviewRendererPayload) {
      setAudioPreviewMessage(
        "No audio preview renderer payload available to copy.",
      );
      return;
    }
  ...
  const copyAudioPreviewSongSheet = async () => {
    if (!audioPreviewSongSheetText.trim()) {
      setAudioPreviewMessage(
        "No audio preview placed songsheet available to copy.",
      );
      return;
    }
  ...
  const copyAudioPreviewSectionGuide = async () => {
    if (!audioPreviewSectionGuideText.trim()) {
      setAudioPreviewMessage(
        "No audio preview section guide available to copy.",
      );
      return;
    }
  ...
  const copyAudioGuidePrompt = async () => {
    ...
    if (!copyText) {
      setChordExtractionMessage(
        "No performance songsheet available for an audio guide prompt.",
      );
      return;
    }
  ...
  const copyPerformanceDesignNotes = async () => {
    ...
    if (!copyText) {
      setChordExtractionMessage(
        "No performance design notes available to copy.",
      );
      return;
    }
    ...
    return [
      ...
      ...(audioReadinessText
        ? [
            ...
            "",
            audioReadinessText.replace(/^AUDIO GUIDE READINESS\s*/i, "").trim(),
            "",
          ]
        : []),
      "",
        ...
        ? [
            ...
            ...(realRenderReadinessSummary.readinessStatus
              ? [
                  ...
                  "",
                  "Blockers:",
                  ...realRenderReadinessSummary.blockers.map(
                    (item) => `- ${item}`,
                  ),
                  "",
                  "Required decisions:",
            ...
            ...(guideTrackRenderRecipeSummary
              ? [
                  ...
                  `Primary bed: ${
                    guideTrackRenderRecipeSummary.musicalBed
                      .primaryInstrument || "Not declared"
                  }`,
                  "",
                  guideTrackRenderRecipeSummary.rendererRequirement
                    ? `Renderer requirement: ${guideTrackRenderRecipeSummary.rendererRequirement}`
                  ...
                  ...(guideTrackRenderRecipeSummary.mixPriorities.length > 0
                    ...
                    : []),
                  ...(guideTrackRenderRecipeSummary.completionCriteria.length >
                  0
                    ? [
                        "Completion criteria:",
            ...
            ...(clickTrackRenderRecipeSummary
              ? [
                  ...
                  ...(clickTrackRenderRecipeSummary.sectionMarkers.enabled
                    ? [
                        "Section markers:",
                        clickTrackRenderRecipeSummary.sectionMarkers
                          .description || "Section markers declared.",
                        "",
                      ]
                    : []),
                  ...(clickTrackRenderRecipeSummary.mixPriorities.length > 0
                    ...
                    : []),
                  ...(clickTrackRenderRecipeSummary.completionCriteria.length >
                  0
                    ? [
                        "Completion criteria:",
                        ...clickTrackRenderRecipeSummary.completionCriteria.map(
            ...
            ...(chordReferenceRenderRecipeSummary
              ? [
                  ...
                  `Voicing: ${
                    chordReferenceRenderRecipeSummary.voicing
                      .primaryInstrument || "Not declared"
                  }${
                    chordReferenceRenderRecipeSummary.voicing.density
                      ? `, ${chordReferenceRenderRecipeSummary.voicing.density}`
                  ...
                  ...(chordReferenceRenderRecipeSummary.mixPriorities.length > 0
                    ...
                    : []),
                  ...(chordReferenceRenderRecipeSummary.completionCriteria
                    .length > 0
                    ? [
                        "Completion criteria:",
                        ...chordReferenceRenderRecipeSummary.completionCriteria.map(
            ...
            ...(vocalGuideRenderRecipeSummary
              ? [
                  ...
                  `Output status: ${vocalGuideRenderRecipeSummary.outputStatus || "Unknown"}`,
                  `Melody source: ${
                    vocalGuideRenderRecipeSummary.melodySource.status ||
                    "Unknown"
                  }`,
                  "",
                  vocalGuideRenderRecipeSummary.rendererRequirement
                    ...
                    : "",
                  "",
                  ...(vocalGuideRenderRecipeSummary.activationRequirements
                    .length > 0
                    ? [
                        "Activation requirements:",
                        ...vocalGuideRenderRecipeSummary.activationRequirements.map(
                  ...
                  ...(vocalGuideRenderRecipeSummary.vocalStyle.defaultReference
                    ? [
                        "Vocal style placeholder:",
                        vocalGuideRenderRecipeSummary.vocalStyle
                          .defaultReference,
                        "",
                      ]
                    : []),
                  ...(vocalGuideRenderRecipeSummary.mixPriorities.length > 0
                    ...
                    : []),
                  ...(vocalGuideRenderRecipeSummary.completionCriteria.length >
                  0
                    ? [
                        "Completion criteria:",
                        ...vocalGuideRenderRecipeSummary.completionCriteria.map(
            ...
            ...(rendererInputContractSummary
              ? [
                  ...
                  rendererInputContractSummary.purpose
                    ...
                    : "",
                  "",
                  ...(rendererInputContractSummary.requiredBeforeRealRender
                    .length > 0
                    ? [
                        "Required before real render:",
                        ...rendererInputContractSummary.requiredBeforeRealRender.map(
            ...
            ...(firstRealRenderPlanSummary
              ? [
                  ...
                  `Audio status: ${firstRealRenderPlanSummary.audioStatus || "Unknown"}`,
                  `Recommended first target: ${
                    firstRealRenderPlanSummary.recommendedFirstTarget ||
                    "Unknown"
                  }`,
                  `Strategy: ${
                    firstRealRenderPlanSummary.rendererStrategy.strategyType ||
                  ...
                  firstRealRenderPlanSummary.recommendedReason
                    ...
                    : "",
                  "",
                  ...(firstRealRenderPlanSummary.firstUnlockRequirements
                    .length > 0
                    ? [
                        "First unlock requirements:",
                        ...firstRealRenderPlanSummary.firstUnlockRequirements.map(
                    ...
                    : []),
                  ...(firstRealRenderPlanSummary.firstValidationChecks.length >
                  0
                    ? [
                        "First validation checks:",
                        ...firstRealRenderPlanSummary.firstValidationChecks.map(
              ...
              ? [
                  ...
                  ...(receivedConfigurationCheck
                    ? [
                        ...
                        ...(receivedConfigurationCheck.missingOrInvalid.length >
                        0
                          ? [
                              "- Missing or invalid:",
            ...
            ...(realRenderConfigurationSummary
              ? [
                  ...
                  `First target: ${
                    ...
                    realRenderConfigurationSummary.firstTarget.status ||
                    "Unknown"
                  })`,
                  "",
                  `Renderer: ${
                  ...
                  `Format candidate: ${
                    ...
                    realRenderConfigurationSummary.outputFormat.status ||
                    "Unknown"
                  })`,
                  `Format selected: ${
                    realRenderConfigurationSummary.outputFormat
                  ...
                  `Sample-rate candidate: ${
                    ...
                    realRenderConfigurationSummary.sampleRate.status ||
                    "Unknown"
                  })`,
                  `Sample rate selected: ${
                    realRenderConfigurationSummary.sampleRate
                  ...
                  `Storage candidate: ${
                    realRenderConfigurationSummary.storage
                      .recommendedFirstProvider || "not declared"
                  } (${
                    realRenderConfigurationSummary.storage.status || "Unknown"
                  })`,
                  `Storage selected: ${
  ...
  const buildAudioGuidePromptCopyText = () => {
    ...
    return [
      ...
      ...(intentRows.length > 0
        ...
        : []),
      ...(guideTrackPlanText ? ["", guideTrackPlanText] : []),
      songsheetText,
    ].join("\n");
  };
  ...
  const getFullPackAudioPreviewStatus = () => {
    ...
    const hasValidatedRendererPayload =
      isAudioPreviewRendererPayloadValidated();
      ...
      .map((item, index) => ({
        ...
        chordPlacementCount:
          typeof item.chordPlacementCount === "number"
            ? item.chordPlacementCount
            : 0,
        firstLyric: typeof item.firstLyric === "string" ? item.firstLyric : "",
        lastLyric: typeof item.lastLyric === "string" ? item.lastLyric : "",
    ...
    return Object.entries(expectedOutputs)
      ...
      .map(([key, value]) => {
        ...
        return {
          ...
          role: typeof output.role === "string" ? output.role : "",
          description:
          ...
          format: typeof output.format === "string" ? output.format : "unknown",
          url:
      ...
      <div className={wrapperClassName}>
        ...
        >
          ...
          <div className="mt-2 text-xs text-yellow-100/80">
            Blocked route received-contract and received-configuration checks
            may pass, but they only verify safe blocked-route inputs; real audio
            rendering remains locked until renderer, format, storage, and
            execution are deliberately enabled.
          </div>
        </div>
    ...
    return selectedOutputs
      .map((output) => {
        ...
        return {
          ...
          label:
            typeof target.label === "string" ? target.label : "Unnamed target",
          priority:
  ...
  const getDryRunGuideTrackRenderRecipeSummary = () => {
    ...
    const guideTrackRenderRecipe =
      ...
      !Array.isArray(dryRunArtifactPackage.guideTrackRenderRecipe)
        ? (dryRunArtifactPackage.guideTrackRenderRecipe as Record<
            string,
            unknown
          >)
        : null;
    ...
    return {
      ...
      chordHandling: {
        source:
          typeof chordHandling?.source === "string" ? chordHandling.source : "",
        description:
      ...
      vocalGuide: {
        status: typeof vocalGuide?.status === "string" ? vocalGuide.status : "",
        description:
  ...
  const getDryRunClickTrackRenderRecipeSummary = () => {
    ...
    const clickTrackRenderRecipe =
      ...
      !Array.isArray(dryRunArtifactPackage.clickTrackRenderRecipe)
        ? (dryRunArtifactPackage.clickTrackRenderRecipe as Record<
            string,
            unknown
          >)
        : null;
  ...
  const getDryRunVocalGuideRenderRecipeSummary = () => {
    ...
    return {
      ...
      vocalStyle: {
        status: typeof vocalStyle?.status === "string" ? vocalStyle.status : "",
        defaultReference:
  ...
  const getDryRunRendererInputContractSummary = () => {
    ...
    const rendererInputContract =
      ...
      !Array.isArray(dryRunArtifactPackage.rendererInputContract)
        ? (dryRunArtifactPackage.rendererInputContract as Record<
            string,
            unknown
          >)
        : null;
      ...
      expectedBlockedResponse: {
        ...
        receivedContractSummary: {
          hasRendererInputContract:
            ...
            typeof expectedBlockedResponse.receivedContractSummary ===
              "object" &&
  ...
  const getDryRunRealRenderConfigurationSummary = () => {
    ...
    return {
      ...
      sampleRate: {
        status: typeof sampleRate?.status === "string" ? sampleRate.status : "",
        recommendedFirstSampleRateHz:
        ...
        selectedSampleRateHz:
          typeof sampleRate?.selectedSampleRateHz === "number"
            ...
            : null,
        reason: typeof sampleRate?.reason === "string" ? sampleRate.reason : "",
        mustRemainBlockedUntil: Array.isArray(
      ...
      storage: {
        ...
        selectedProvider:
          typeof storage?.selectedProvider === "string"
            ...
            : null,
        reason: typeof storage?.reason === "string" ? storage.reason : "",
  ...
  const getAudioPreviewResultStatus = () => {
    if (!audioPreviewPlan) {
      return {
        ...
        detail:
          "Request audio preview to create the planner output and renderer payload.",
        tone: "missing",
      };
    }
  ...
  const getAudioPreviewRendererPayloadSummary = () => {
    ...
    const songsheetLines = Array.isArray(
  ...
  const getAudioPreviewChecklist = () => {
    ...
    const realRenderReadinessSummary = getDryRunRealRenderReadinessSummary();
    const hasRealRenderReadiness = Boolean(
      realRenderReadinessSummary.readinessStatus,
    );
  ...
  const getAudioPreviewSpecStatus = () => {
    ...
    if (!previewSpec) {
      return {
        ...
        detail:
          "Generate or load chord data with performance intent or songsheet lines.",
        isValid: false,
      };
    }
    ...
    try {
      ...
      return {
        ...
        detail:
          "The audio preview spec can be copied as machine-readable JSON.",
        isValid: true,
      };
    } catch {
      return {
  ...
  const getNextChordWorkflowAction = () => {
    ...
    const hasPreviewRequest = Boolean(
      audioPreviewPlan || audioPreviewRenderPrompt,
    );
    const sourceMatch = getPlacedSongsheetSourceMatch();
    const serverValidation = getSongsheetServerValidation();
    const sourceCoverage = getPlacedSongsheetSourceCoverage();
  ...
  const getChordWorkflowStatus = () => {
    ...
    const hasSongsheetReviewWarnings =
      ...
      serverValidation.rejectedLineCount > 0 ||
      sourceCoverage.missingLineCount > 0;
    const activeProcess = generatingChords
    ...
    return {
      ...
      detail:
        activeProcess ||
        `${completeCount} of ${steps.length} workflow steps are complete.`,
      activeProcess,
      steps,
    };
  };
    ...
    return [
      ...
      ...(guideTrackPlanText
        ? [
            "GUIDE TRACK PLAN",
            "",
            guideTrackPlanText.replace(/^GUIDE TRACK PLAN\s*/i, "").trim(),
          ]
        : []),
    ]
      .filter((line, index, lines) => {
  ...
  const getPlacedSongSheetQuality = () => {
    ...
    const linesWithChords = lines.filter(
  ...
  const renderPlacedSongSheetLine = (line: PlacedSongSheetLine) => {
    ...
    return [chordCharacters.join("").trimEnd(), lyric];
  };
  ...
  const getChordEditorStatus = () => {
    ...
    const summaryRows = getChordSummaryRows(
      getUsableChordDataFromEditor() || chords,
    );
    ...
    if (!usableChordData) {
      return {
        ...
        detail:
          "The editor has text, but it is not valid usable chord JSON yet.",
      };
    }

    return {
      label: activeChordVersionId
        ? "Saved chord version loaded"
        : "Unsaved chord draft",
  ...
  const audioPreviewRendererPayloadSummary =
    getAudioPreviewRendererPayloadSummary();
  ...
  const guideTrackPlanRows = getGuideTrackPlanRows(
    getChordDataFromEditorJson(),
  );
  ...
  const keyChordConsistency = getKeyChordConsistency(
    getChordDataFromEditorJson(),
  );
  ...
  const generateChords = async () => {
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "Chord generation did not return a valid chord object.",
        );
      }
      ...
      setChordTransposeSemitones(0);
      setChordExtractionMessage(
        "Chords generated. Review and save when ready.",
      );
      ...
      setChordExtractionMessage(
        error instanceof Error ? error.message : "Failed to generate chords.",
      );
    } finally {
  ...
  const saveChords = async () => {
    try {
      ...
      const suspiciousChordText = getChordStringValues(chordsToSave).some(
        (value) => /([a-zA-Z])\1{7,}/.test(value),
      );
      ...
      <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center p-6">
        <div className="w-full max-w-md rounded-xl bg-gray-800 p-6 shadow-xl border border-gray-700">
          ...
          <div className="flex flex-col gap-3">
            ...
            <label className="text-sm text-gray-300 mt-3">
              Verification code
            </label>
            <input
              value={otp}
  ...
  const getDiffLines = (left: string, right: string) => {
    ...
    const wordsForLine = (value: string) =>
      normaliseLineForDiff(value).split(" ").filter(Boolean);
    ...
    const alignLines = (leftLines: SongLine[], rightLines: SongLine[]) => {
      ...
      while (leftIndex > 0 || rightIndex > 0) {
        ...
        const rightOriginalIndex =
          rightLines[rightIndex - 1]?.lineIndex ?? null;
        ...
        if (
          ...
          rows.unshift({
            ...
            right: rightLine,
            changed:
              normaliseLineForDiff(leftLine) !==
              normaliseLineForDiff(rightLine),
    ...
    const leftBlocksByKey = new Map(
      leftBlocks.map((block) => [block.key, block]),
    );
    const rightBlocksByKey = new Map(
      rightBlocks.map((block) => [block.key, block]),
    );
  ...
  const getWordDiffParts = (left: string, right: string) => {
    ...
    for (let leftIndex = leftTokens.length - 1; leftIndex >= 0; leftIndex--) {
      for (
        ...
        if (
          leftTokens[leftIndex].comparable ===
          rightTokens[rightIndex].comparable
        ) {
  ...
  const removeChordsFromRewriteSource = () => {
    ...
    setRewriteSectionName("");
    setRewriteMessage(
      "Chord lines removed. Please re-select the section to rewrite.",
    );
  ...
  const runRewriteLab = async () => {
    ...
    if (hasChordLinesInRewriteSource) {
      setRewriteMessage(
        "Chords detected. Please remove chord lines before rewriting.",
      );
      return;
    }
    ...
    if (rewriteSectionOnly && !rewriteSectionName) {
      setRewriteMessage(
        "Please choose a section before running a section rewrite.",
      );
      return;
    }
...
`
        ...
        const testSection = rewriteSectionOnly
          ? extractSectionTextStrict(rewritten, rewriteSectionName, (line) =>
      ...
      if (!rewritten || !rewritten.trim()) {
        throw new Error(
          "Rewrite failed — AI could not produce a valid version. Try again.",
        );
      }
      ...
      if (
        normaliseForCompare(finalText) === normaliseForCompare(fullSourceText)
      ) {
        setRewriteMessage(
      ...
      const rewriteVoiceLabel =
        rewriteVoiceOptions.find((option) => option.id === rewriteVoice)
          ?.label || "Natural British";
      ...
      setTimeout(() => {
        document.getElementById("rewrite-compare-preview")?.scrollIntoView({
          behavior: "smooth",
          block: "start",
        });
      }, 150);
  ...
  const panelsMatch = compareLeftText.trim() === compareRightText.trim();
